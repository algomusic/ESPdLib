/*
 * SimpleSequence - ESPdLib Example
 *
 * Plays a rhythmic sequence using a phasor through a low-pass filter
 * with an amplitude envelope triggered by a metro at 500ms.
 * Three potentiometers control frequency, amplitude, and filter cutoff.
 *
 * Patch signal flow:
 *   [loadbang] -> [tgl] -> [metro 500] -> [vline~ envelope] -> [*~]
 *   [r freq] -> [phasor~] -> [lop~ cutoff] -> [*~ envelope] -> [*~ amp] -> [dac~]
 *
 * Hardware:
 *   - ESP32 board
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("SimpleSequence - ESPdLib Example");
    ...
    if (!Pd.openPatch("simple-sequence.pd")) {
